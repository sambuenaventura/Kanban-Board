            ...
            @foreach ($plans as $plan)
                <div class="relative bg-white border border-gray-200 rounded-lg shadow-sm divide-y divide-gray-200">
                    ...
                    <div class="p-6">
                        ...
                        @if ($plan['name'] != 'Basic')
                            <a href="{{ $plan['route'] }}" class="mt-8 block w-full bg-indigo-600 border border-transparent rounded-md py-2 text-sm font-semibold text-white text-center hover:bg-indigo-700">Get {{ $plan['name'] }}</a>
                        @else
                            <a href="javascript:void(0);" class="mt-8 block w-full bg-gray-400 border border-transparent rounded-md py-2 text-sm font-semibold text-white text-center cursor-not-allowed">Free</a>
                        @endif
                    </div>
